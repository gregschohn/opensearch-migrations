...
spec: {
		...
		let DS=(#StepTemplate & {
          	...
          	name: "endpoint"
            valueFrom: {
            	expression: "steps['\(CS.name)'].outputs.parameters['endpoint'] + ':' + \(_paramsWithTemplatePathsMap.frontsidePort.exprInputPath)"
            }
          }]
        }
				steps: [[
					{
						...
						template: N
						arguments: {
								parameters: (#ProxyInputsIntoArguments & {#in: _paramsWithTemplatePathsMap }).out
							}
					}
				]]
		})
